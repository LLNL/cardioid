...
#define loadV(x,y,z) vec_xl( 2*(z) + 2*Lkk*(y) + 2*LkkLjj*(x) , baseV )
#define loadS(x,y,z,dir) vec_xl(2*(z) + 2*(Lkk-1)*(y) + 2*Lkk_1Ljj_1*(x) + 2*Lkk_1Ljj_1Lii_1 * (dir) , baseS)
#define storeV(x,y,z,v) vec_xst(v,2*(z) + 2*Lkk*(y) + 2*LkkLjj*(x),baseST)
...
void simd_diff_cpu_hoist(Real* __restrict__ d_psi, Real* __restrict__ d_npsi, Real* __restrict__ d_sigmaX, Real* __restrict__ d_sigmaY, Real* __restrict__ d_sigmaZ,int Lii, int Ljj, int Lkk)
{

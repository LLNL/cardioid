...
# This script creates .ddcMD visualization files (can be opened in Visit) from full resolution anatomy#* files.  It may create multiple .ddcMD files, in which case, each one will have to be openend in Visit one by one to visualize entire heart anatomy.  Companion python script "loadddcMDsInVisit.py" aids in automating importing .ddcMD files into Visit.
...
if len(sys.argv) < 3:
  print '\n\nERROR, usage:  anatomy2ddcMDvis [anatomy files base name] [maximum number of anatomy files you want assigned to each .ddcMD visualization file]\n\n*(smaller number gives more .ddcMD files, but each one takes less time to visualize in Visit, which avoids intrinsic 30 minute execution timeout in Visit)\n\n'
  print 'e.g., anatomy2ddcMDvis anatomy 1\n\n'
  print 'Note:  using 1 for max. number of anatomy files per .ddcMD visualization file gives best performance in Visit'
  exit(1)
...
# Gather header data from existing anatomy#000000
print 'Gathering header data from existing anatomy#000000, and copying its data only (not header) to '+dirname+'/'+fb+'#000000'
myf = fb+'#000000'
fin = open(myf,'r')
...
if nbatch <= 1:
    ...
    print '\nSwitching nx and nz, as Visit expects them to be switched to what we have from these anatomy files\n'
...
for myf in files:
 if len(files) <= 1 or myf != 'anatomy#000000':
#  print 'ct is '+str(ct)
...
print '6)  If number of files per .ddcMD visualization file is set to 1, each .ddcMD files takes about 1 minute to visualize in Visit.  Setting number of files per .ddcMD visualization file to 1 is best and yields fastest drawing of viz in Visit\n'
print 'BONUS:  the script loadddcMDsInVisit.py accomplishes steps 2 and  4-6 if you run it in the Visit GUI, but you will need to write in your expression for step 3, and of course, edit the parameters in the script to fit your own needs'

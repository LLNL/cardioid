...
double TT04_CellML_Epi::constants_[46];
double TT04_CellML_Epi::defaultState_[17];
...
TT04_CellML_Epi::TT04_CellML_Epi()
{
...
double TT04_CellML_Epi::calc(double dt, double Vm, double iStim)
{
   double dummy  = 0.;
   double rates[17];
   double algebraic[67];
...
/** Everything below this point is the TT04 epi model generated code
 *  from the CellML web site (retrieved 4-Nov-2011).  This code has been
 *  altered as follows:
 *
 ...
 *
 *  3. The computation of ALGEBRAIC[0] in computeRates is commented
 *  out.  ALGEBRAIC[0] corresponds to the stimulus current.  It is
 *  expected that ALGEBRAIC[0] will be loaded with a physically relevant
 *  value by the caller.  Note that VOI is used only in the calculation
 *  of ALGEBRAIC[0] so it effectively becomes an unused parameter as do
 ...
 *  ALGEBRAIC[0]).  This is not the convention that we use in our code.
 ...
 * ALGEBRAIC[47] is alpha_K1 in component inward_rectifier_potassium_current (per_millisecond).
 * ALGEBRAIC[48] is beta_K1 in component inward_rectifier_potassium_current (per_millisecond).

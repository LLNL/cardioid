...
{
   double mhu = 1/(1 + exp (-(Vm + 39.57)/9.871)); 
   return mhu
}
double mTauR(double Vm, void *)
{
double tauR  = 6.765 + exp((Vm + 11.64)/34.77)  + 8.552 * exp (-(Vm + 77.42)/5.955); 
return tauR; 
}
//Note: hFastMhu = hSlowMhu = jMhu = jCaMKMhu = hMhu; 
double  hMhu(double Vm, void *)
{
 double mhu = 1/(1 + exp ((Vm + 82.9)/6.086);
 return mhu; 
}
double hFastTauR(double Vm, void *)
{
 double tauR = 1.432*e-5 * exp(-(Vm + 1.196)/6.285) + 6.149 * exp((Vm + 0.5096)/20.27);
 return tauR; 
 
}
double hSlowTauR(double Vm, void *)
{
 double tauR = 0.009764 * exp (-(Vm + 17.95)/28.05) + 0.3343 * exp((Vm+5.730)/56.66);
 return tauR; 
}
double jTauR(double Vm, void *) 
{ 
double tau = = 2.038 + 1.0/(0.02136*exp(-(V + 100.6)/8.281) + 0.3052*exp((V + 0.9941)/38.45)); 
return 1/tauR; 
}
double CaMKMhu(double Vm, void *) 
{ 
   double mhu=1.0/(1.0+exp((Vm+89.1)/6.086));
   return mhu;
}
double CaMKSlowTauR(double Vm, void *) 
{ 
   double tauR = hSlowTauR(Vm, void*)/3.0;
   return tauR;
}
double CaMKTauR(double Vm, void *) 
{ 
   double tauR = jTauR(Vm, void*)/1.46;
   return tauR;
}
double mLMhu(double Vm, void *) 
{ 
   double mhu = 1.0/(1.0+exp(-(Vm+42.85)/5.264));
   return mhu;
}
// mLTauR = mTauR; 

double hLMhu(double Vm, void *) 
{ 
   double mhu = 1.0/(1.0+exp((Vm+87.61)/7.488));
   return mhu;
}
double hLTauR(double Vm, void *) 
{ 
   double tauR = 0.005; 
   return tauR;
}
double hLCaMKMhu(double Vm, void *) 
{ 
   double mhu = 1.0/(1.0+exp((Vm+93.81)/7.488));
   return mhu;
}
double hLCaMKTauR(double Vm, void *) 
{ 
   double tauR  =  hLTauR(Vm,NULL)/3.0;
   return tauR;
}
double aMhu(double Vm, void *) 
{ 
   double mhu = 1.0/(1.0+exp(-(Vm+14.34)/14.82));
   return mhu;
}
double aTauR(double Vm, void *) 
{ 
   double tauR  =  (1.0/2.089)*sigm(-(Vm-18.41)/29.38)+3.5*sigm((Vm+100)/29.38);
   return tauR;
}
double iMhu(double Vm, void *) 
{ 
   double mhu = sigm((Vm+43.94)/5.711);
   return mhu;
}
double iFastTauR(double Vm, void *) 
{ 
   double tau  =  4.562 + 1.0/(0.3933*exp(-(Vm+100)/100) + 0.08004*exp((Vm+50)/16.59)); 
   return 1.0/tau;
}
double iSlowTauR(double Vm, void *) 
{ 
   double tau  = 23.62  + 1.0/(0.01416*exp(-(Vm+ 96.52)/59.05) + 1.7808e-8*exp((Vm+114.1)/8.079)); 
   return 1.0/tau;
}
double aCaMKMhu(double Vm, void *) 
{ 
   double mhu = sigm(-(Vm+24.39)/14.82);
   return mhu;
}
//aCaMKTauR = aTauR 
}

...
static char*  readPheader(char* filename);
static int    fillBuffer(char* buf, int tid, const PFILE* file);
...
static int    computeMaxReadBuf(const PFILE* file);
...
static off_t  readStart(int iFile, int tid, int* readLen, const PFILE*);
...
int Pio_groupSetup(PFILE* file)
{
	...
	{
		file->sizegroup ++;
		file->ngroup = (file->size/file->sizegroup) +1;
	}
	return setupIoIds(file);
}


void PioSet(PFILE*file, char *string, ... )
{
	va_list ap;
	va_start(ap, string);
...
int Pprintf(PFILE* file, const char* fmt, ...)
{
	...
	int size = 1024;
	va_start(ap, fmt);
	int n = vsnprintf(line, size, fmt, ap);
	va_end(ap);
	bufferAppend(file, line, n);
	return n;
}
...
void Popen_forRead(PFILE* file)
{
   ...
   if (file->id == 0)
   {
      header = readPheader(file->name);
      lhdr = strlen(header);
      for (int ii=0; ii<lhdr; ++ii)
	 if (header[ii] == '\n')
	    header[ii] = ' ';
   }
   MPI_Bcast(&lhdr, 1, MPI_INT, 0, file->comm);
   if (file->id != 0)
      header = (char*) ddcMalloc(lhdr+1);
   MPI_Bcast(header, lhdr+1, MPI_CHAR, 0, file->comm);
   ...
   object_get(hobj, "nrecord",  &file->numberRecords, ULL,    1, "0");
   ...
   file->checksum = checksum;
	...
	if (file->groupToHandle >= 0)
	{
		int maxReadBuf = computeMaxReadBuf(file);
...
char* readPheader(char* filename)
{
...
/*
   ...
   fgets(line, 1024, masterFile);
   while (strlen(line) > 1) fgets(line, 1024, masterFile);
...
int fillBuffer(char* buf, int tid, const PFILE* file)
{
...
static off_t readStart(int iFile, int tid, int* readLen, const PFILE* file)
{
   int gid = groupId(tid, file);

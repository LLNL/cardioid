...
#include "SimpleOHaraRudyTT06.hh"
...
#define setDefault(name, value) objectGet(obj, #name, reaction->name, #value)
   ...
   {
      SimpleOHaraRudyTT06::ThisReaction* reaction = new SimpleOHaraRudyTT06::ThisReaction(anatomy);
                       ...
                       const vector<double>& iStim , VectorDouble32& dVm)
{
   for (unsigned ii=0; ii<nCells_; ++ii)
   {
      ...
      const double mTT2=state_[ii].mTT2;
      const double hTT2=state_[ii].hTT2;
      const double jTT2=state_[ii].jTT2;
      ...
      const double diff_mTT2=(mTT2_inf-mTT2)/tau_mTT2;
      const double diff_hTT2=(hTT2_inf-hTT2)/tau_hTT2;
      const double diff_jTT2=(jTT2_inf-jTT2)/tau_jTT2;
      const double GNaTT2=14.838;
      const double INa=GNaTT2*(v-ENa)*mTT2*mTT2*mTT2*hTT2*jTT2;
...
#define CHECK_BLOWUP(x) do { if (!isfinite(x)) { fprintf(stderr, "Error in node %d, variable " #x " = %g\n", ii, (x)); foundError=true; } } while(0)
         ...
         CHECK_BLOWUP(diff_mTT2);
         CHECK_BLOWUP(diff_hTT2);
         CHECK_BLOWUP(diff_jTT2);
...
#undef PRINT_STATE
      ...
      state_[ii].mTT2 = mTT2_inf-(mTT2_inf-mTT2)*exp(-dt/tau_mTT2);
      state_[ii].hTT2 = hTT2_inf-(hTT2_inf-hTT2)*exp(-dt/tau_hTT2);
      state_[ii].jTT2 = jTT2_inf-(jTT2_inf-jTT2)*exp(-dt/tau_jTT2);

...
#include <iostream>
#include "Anatomy.hh"
#include "OHaraRudyFunc.hh"
#include "OHaraRudyGates.h"
#include "OHaraRudyNonGates.h"
#include "pade.hh"
...
#define NGATES 28
#define NCONCENTRATIONS 8
#define NMISC  4

using namespace std;
using namespace PerformanceTimers;
using namespace OHaraRudyFunc;
int workBundle(int index, int nItems, int nGroups , int mod, int& offset);

static   UPDATEGATE gateEq[] ={ update_m,         update_hFast,       update_hSlow,  update_j,      update_CaMKSlow, udpate_jCaMK,   update_mL,          
                                update_hL,        update_hLCaMK,      update_a,      update_iFast,  update_iSlow,    update_aCaMK,   update_iCaMKFast, 
                                update_iCaMKSlow, update_d,           update_fFast,  update_fSlow,  update_fCaFast,  update_fCaSlow, update_jCa,
                                update_fCaMKFast, update_fCaCaMKFast, update_XrFast, update_XrSlow, update_Xs1,      update_Xs2,     udpate_XK1 }; 
static double *mhuX[NGATES]; 
static double *tauRX[NGATES]; 
static double *gateX[NGATES]; 
static UPDATEGATE gateEqX[NGATES]; 
...
{
      const char *fitName[] ={ "mMhu", "mTauR", "hMhu", "hTauR", "hTauRMod", "jMhu", "jTauR", "jTauRMod", "Xr1Mhu", "Xr1TauR", "Xr2Mhu", "Xr2TauR", 
...
// operator. 
OHaraRudy_Reaction::OHaraRudy_Reaction(double dt, Anatomy& anatomy, OHaraRudy_ReactionParms& parms, const ThreadTeam& group)
                 : nCells_(anatomy.nLocal()), group_(group)
{
   ...
   {
      initialized = true;
      OHaraRudyFunc::initCnst();
      initExp(); 
      int pid;
      MPI_Comm_rank(MPI_COMM_WORLD,&pid);	  
      string fCassFormName[] = { "OHaraRudy", "RICE"}; 
   ...
   if ( fastGates   == 0) update_gate_   =OHaraRudyFunc::updateGate;
//   if ( fastGates   >= 1) update_gate_   =OHaraRudyFunc::updateGateFast;
...
void OHaraRudy_Reaction::scaleCurrents(vector <double> currentScales)
{
...
void OHaraRudy_Reaction::mkCellTypeParms_(Anatomy& anatomy,OHaraRudy_ReactionParms& parms)
{
...
void OHaraRudy_Reaction::mkCellTypeVector_(Anatomy& anatomy, OHaraRudy_ReactionParms& parms)
{
...
void OHaraRudy_Reaction::mkState_(OHaraRudy_ReactionParms& parms)
{
...
void OHaraRudy_Reaction::mkFitParms_(OHaraRudy_ReactionParms& parms)
{
   fit_ = parms.fit; 
   if (fit_ == NULL ) 
   {
...
void OHaraRudy_Reaction::mkWorkBundles_(OHaraRudy_ReactionParms& parms)
{
...
OHaraRudy_Reaction::~OHaraRudy_Reaction()
{
   free(stateBuffer_);
   delete fit_;
}
// void OHaraRudy_Reaction::writeStateDev(int loop)
// {
...
int OHaraRudy_Reaction::nonGateWorkPartition_(int& offset)
{
   offset=0; 
...
void OHaraRudy_Reaction::calc(double dt, const VectorDouble32& Vm, const vector<double>& iStim, VectorDouble32& dVm)
{
...
void OHaraRudy_Reaction::updateNonGate(double dt, const VectorDouble32& Vm, VectorDouble32& dVR)
{
   int offset,nCells; 
...
void OHaraRudy_Reaction::updateGate(double dt, const VectorDouble32& Vm)
{
   int teamRank = group_.teamRank();

   OHaraRudyFunc::WORK work = gateWork_[teamRank]; 
...
void OHaraRudy_Reaction::getCheckpointInfo(vector<string>& name,
...
double OHaraRudy_Reaction::getValue(int iCell, int handle) const
{
   assert(handle >= 0);
...
HandleMap& OHaraRudy_Reaction::getHandleMap() const
{

   ...
   const vector<double>& Vm, vector<double>& dVm, double *recv_buf_, int nLocal)
{
   ...
   //   Array3d<double> tmp_dVm(VmTmp->nx(),VmTmp->ny(),VmTmp->nz());
     ...
     boundaryFDLaplacianSaleheen98SumPhi_All_simd_thread(start,end-start,VmTmp,tmp_dVm.cBlock());
    
     start = VmTmp->tupleToIndex(1,1,8);
     end = VmTmp->tupleToIndex(VmTmp->nx()-2,VmTmp->ny()-2,VmTmp->nz());
...
void
Saleheen98PrecomputeDiffusion::boundaryFDLaplacianSaleheen98SumPhi_All_simd_thread(const uint32_t start,const int32_t chunk_size, Array3d<double>* VmTmp, double* out)
{
  int ii;

...
double TT04Dev::constants_[52];
double TT04Dev::defaultState_[17];
...
namespace
{
   ...
   void
      computeRates(double VOI, double* CONSTANTS, double* RATES, double* STATES, double* ALGEBRAIC,int cellType);
}

   

TT04Dev::TT04Dev(int cellType)
{
...
double TT04Dev::calc(double dt, double Vm, double iStim)
{
   double dummy  = 0.;
   double rates[17];
   double algebraic[67];
   algebraic[0] = iStim;
   states_[0] = Vm;
   computeRates(dummy, constants_, rates, states_, algebraic,cellType_);

   for (unsigned ii=1; ii<7; ++ii) states_[ii] += rates[ii] * dt;
   states_[7] =  algebraic[4] -(algebraic[4]-states_[7])*exp(-dt/algebraic[39]);
   for (unsigned ii=8; ii<17; ++ii) states_[ii] += rates[ii] * dt;
...
ALGEBRAIC[57] =  CONSTANTS[49+cellType]*STATES[14]*STATES[13]*(STATES[0] - ALGEBRAIC[26]);
...
ALGEBRAIC[52] =  CONSTANTS[46+cellType]*(pow(STATES[6], 2.00000))*(STATES[0] - ALGEBRAIC[35]);

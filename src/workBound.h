...
typedef  struct partition_info_st { int nD, nR, nT, nB, height, dz4, cost, timeR, timeD;} PARTITION_INFO;
...
typedef  struct balancer_st { int rank,  nRank, nTasks, nC, nx, ny, nz, dx, dy, dz, NX, NY, NXYz,nCellLocal,nCellGlobal; 
  				 double alphaWork; 
                                 double alphaBalance; 
    				 double timeRef; 
                                 COLUMN *columns;
...
BALANCER buildBalancer(int nCellGlobal, int nx,int ny,int nz,int dx,int dy,int dz, int  nTasks, int nC, double alpha, int printStat, MPI_Comm comm) ;
